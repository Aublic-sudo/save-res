...
from utils.func import get_user_data_key, get_user_data, save_user_data, users_collection
...
async def send_settings_message(chat_id, user_id):
    ...
    current_chat = ud.get('chat_id', 'Not Set (PM)') if ud else 'Not Set (PM)'
    current_tag = ud.get('rename_tag', 'None') if ud else 'None'
    current_caption = (ud.get('caption', 'None')[:40] + '...') if ud and ud.get('caption') and len(ud.get('caption')) > 40 else (ud.get('caption', 'None') if ud else 'None')
    has_session = '✅ Active' if (ud and ud.get('session_string')) else '❌ None (/login)'
    has_thumb = '✅ Configured' if os.path.exists(f'{user_id}.jpg') else '❌ Default'

    summary_text = (
        f"⚙️ **Personalized Bot Settings:**\n\n"
        f"🎯 **Destination Chat/Topic:** `{current_chat}`\n"
        f"🏷️ **Rename Tag:** `{current_tag}`\n"
        f"📋 **Caption:** `{current_caption}`\n"
        f"🖼️ **Custom Thumbnail:** {has_thumb}\n"
        f"🔑 **User Session:** {has_session}\n\n"
        f"👇 *Click an option below to customize:*"
    )
    ...
    buttons = [
        ...
        [
            Button.url('🆘 Report Errors', 'https://t.me/RixieHQ')
        ]
    ]
    await gf.send_message(chat_id, summary_text, buttons=buttons)
...
async def handle_setchat(event, user_id):
    try:
        ...
        if '/' in chat_id:
            ...
            cid = parts[0].strip()
            tid = parts[1].strip()
            await save_user_data(user_id, 'chat_id', f"{cid}/{tid}")
            await event.respond(
                f"✅ **Destination Topic Configured!**\n\n"
                f"📢 **Channel/Group ID:** `{cid}`\n"
                f"📂 **Topic ID:** `{tid}`\n\n"
                f"All future batch & single uploads will be delivered directly into this topic!"
            )
        else:
            await save_user_data(user_id, 'chat_id', chat_id)
            await event.respond(f"✅ Chat ID set to: `{chat_id}`\n\nAll uploads will be delivered to this chat.")
    except Exception as e:
